...
if ($thistab == 'last_year') {
    ...
    for ($i = 1; $i < 13; $i++) { //for each month
        ...
        $data[$date_from->format('M-Y')] = array('sla'=>$sla, 'raw_data'=>sprintf('total tickets: %d, M:%d, E:%d, U:%d', $tickets_matched, $M, $E, $U));
    }
    ...
    $data[$last_month->format('M-Y')] = array('sla'=>$sla, 'raw_data'=>sprintf('total tickets: %d, M:%d, E:%d, U:%d', $tickets_matched, $M, $E, $U));;
    ...
    $data[$date->format('M-Y')] = array('sla'=>$sla, 'raw_data'=>sprintf('total tickets: %d, M:%d, E:%d, U:%d', $tickets_matched, $M, $E, $U));
    ...
    if ($_GET && $_GET['startDate'] && $_GET['endDate']) {
        ...
        if ($months) {
            for ($i = 1; $i <= $months; $i++) {
                ...
                $sql = 'SELECT * FROM ' . TICKET_TABLE . ' WHERE client_id=' . db_input($client_id) . ' AND created BETWEEN ' . db_input($date_from->format('Y-m-d H:i:s')) . ' AND ' . db_input($date_to->format('Y-m-d H:i:s'));
                $res = db_query($sql);
