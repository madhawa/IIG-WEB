...
if (!$errors && ($id = $_REQUEST['id'] ? $_REQUEST['id'] : $_POST['ticket_id']) && is_numeric($id)) {
    ...
    if (!$ticket or !$ticket->getId()) {
        ...
        //$errors['err'] = 'Unknown ticket ID#' . $id.' probably deleted just before'; //Sucker...invalid id
    }
    ...
    foreach( $clients as $each_client ) {
        $cs[] = array('client_id'=>$each_client['client_id'], 'client_name'=>$each_client['client_name']);
    }
    $clients_json = json_encode($cs);
...
if ($_POST && !$errors):
    ...
    if ($ticket && $ticket->getId()) {
        ...
        switch (strtolower($_POST['a'])):
            case 'update':
                if ($ticket->update_ticket($_POST, $errors)) {
                    $msg = 'Update Success!';
                }
                break;
            case 'reply':
            ...
            case 'postnote':
                ...
                $ticket->addInternalCc(Format::striptags(trim(trim($_POST['more_cc']), ',')));
                if (!$errors && $ticket->postNote($_POST['title'], $_POST['note'], true, $thisuser->getId())) {
                ...
                switch (strtolower($_POST['do'])):
                    ...
                    case 'close':
                        if (!$thisuser->isadmin() && !$thisuser->canCloseTickets()) {
                            ...
                            if ($ticket->close($_POST['sla_claim'])) {
        ...
        switch ($_POST['a']) {
            ...
            case 'select_client':
                ...
                $selected_client = new Client($client_id);
