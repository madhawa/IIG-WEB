...
/* * *******************************************************************
  ...
  Handles all admin related pages and engineering staff pages, with permission sets
  if requests are exclusive to admin, not engineering staffs: it checks permissions
...
if (!$thisuser or !$thisuser->isEngineer()) {
...
if ($_POST && $_REQUEST['t'] && !$errors):
    ...
    switch (strtolower($_REQUEST['t'])):
        ...
        case '1asiaahl':
            ...
            require_once(INCLUDE_DIR . '1asiaahl.inc.php');
            if (asiaahl($_POST, $errors)) {
                $msg = 'success';
            }
            break;
        case 'pref':
            ...
            if ($cfg->updatePref($_POST, $errors)) {
                ...
                if ( $_POST['more_ticket_alert_email'] ) {
                    $sql = 'UPDATE ' . CUSTOM_PREF_TABLE . ' SET more_ticket_alert_email=' . db_input($_POST['more_ticket_alert_email']) . ' WHERE id='.db_input(1);
                    if (db_query($sql)) {
                        $msg .= ' ticket alert emails saved Successfully';
                    } else {
                        mysql_query($sql) or die(mysql_error());
                        $errors['err'] = 'Error in saving ticket alert emails';
                    }
                }
        ...
        case 'client':
            ...
            switch ($do) {
                case 'search':
                    //print_r($_POST);
                    $criteria;
                    $criteria['what'] = $_POST['search_client'];
                    $criteria['type'] = $_POST['search_type'];
                    break;
                case 'update':
                    ...
                    break;
                case 'create':
                    if ( !Client::is_uniq_name($_POST['client_name']) ) { //name is not unique
                        $errors['err'] = 'name already in the databse, choose another name';
                    } else { // name unique
                ...
                case 'new_staff':
                    ...
                    break;
                case 'update_staff':
                      echo   'entered staff update portion';
                    if (Client::updateClientStaff($_POST, $errors)) {
                        $msg = 'staff updated successsfully';
                    } else {
                        $errors['err'] = 'Unable to update the staff';
                    }
                    break;
                case 'mass_process':
					$ids = implode(',', $_POST['delete_client']);
					
					foreach ( $_POST['delete_client'] as $cl_id ) {
						$sql = 'DELETE FROM ' . CLIENT_TABLE . ' WHERE client_id=' . db_input($cl_id) . ' AND client_id != ' . $thisuser->getId();
						mysql_query($sql) or die(mysql_error());
						//now delete cin number
						$sql_cin = 'DELETE FROM ' . SERVICE_CIN_TABLE . ' WHERE client_id=' . db_input($cl_id) . ' AND client_id != ' . $thisuser->getId();
						mysql_query($sql_cin) or die(mysql_error());
					}
					//$sql = 'DELETE FROM ' . CLIENT_TABLE . ' WHERE client_id IN ( ' . db_input($ids) . ' ) AND client_id != ' . $thisuser->getId();
					//mysql_query($sql) or die(mysql_error());
					//  echo   $sql;
					//$msg = db_affected_rows() . ' users deleted';
					//print_r($_POST['delete_client']);
                    //ok..at this point..look WMA.
					/*
                    if ($_POST['uids'] && is_array($_POST['uids'])) {
                        $ids = implode(',', $_POST['uids']);
                        $selected = count($_POST['uids']);
                        if (isset($_POST['enable'])) {
                            $sql = 'UPDATE ' . CLIENT_TABLE . ' SET isactive=1,updated=NOW() WHERE isactive=0 AND client_id IN(' . $ids . ')';
                            db_query($sql);
                            $msg = db_affected_rows() . " of  $selected selected users enabled";
                        } elseif (in_array($thisuser->getID(), $_POST['uids'])) {
                            //sucker...watch what you are doing...why don't you just DROP the DB?
                            ...
                            $sql = 'UPDATE ' . CLIENT_TABLE . ' SET isactive=0, updated=NOW() ' .
                                    ' WHERE isactive=1 AND client_id IN(' . $ids . ') AND client_id!=' . $thisuser->getID();
                            db_query($sql);
                            $msg = db_affected_rows() . " of  $selected selected users locked";
                            //Release tickets assigned to the user?? NO? could be a temp thing 
                            // May be auto-release if not logged in for X days? 
                        } elseif (isset($_POST['delete'])) {
                            db_query('DELETE FROM ' . CLIENT_TABLE . ' WHERE client_id IN(' . $ids . ') AND client_id!=' . $thisuser->getID());
                            $msg = db_affected_rows() . " of  $selected selected users deleted";
                            //Demote the user 
                            db_query('UPDATE ' . DEPT_TABLE . ' SET manager_id=0 WHERE manager_id IN(' . $ids . ') ');
                            db_query('UPDATE ' . TICKET_TABLE . ' SET staff_id=0 WHERE staff_id IN(' . $ids . ') ');
                    ...
                    break;
                default:
                    $errors['err'] = 'Uknown command for client!';
            }
            break;
        case 'dept':
...
switch ($thistab) {
    ...
    case 'manage':
        $nav->setTabActive('manage');
        //$nav->addSubMenu(array('desc' => 'MRTG links', 'href' => 'admin.php?t=mrtg', 'iconclass' => 'api'));
        //$nav->addSubMenu(array('desc' => 'Email', 'href' => 'admin.php?t=email', 'iconclass' => 'preferences'));
        //$nav->addSubMenu(array('desc' => 'Staff', 'href' => 'admin.php?t=staff', 'iconclass' => 'attachment'));
       // $nav->addSubMenu(array('desc' => 'Client', 'href' => 'admin.php?t=client', 'iconclass' => 'api'));
       // $nav->addSubMenu(array('desc' => 'Departments', 'href' => 'admin.php?t=depts', 'iconclass' => 'api'));
    ...
    case 'syslog':
        $nav->setTabActive('dashboard');
        $nav->addSubMenu(array('desc' => 'Dashboard', 'href' => 'admin.php?t=dashboard', 'iconclass' => 'syslogs'));
    ...
    case '1asiaahl':
        //  echo   'i was here';
        $nav->setTabActive('1asia-ahl.com');
        $page = '1asiaahl.tpl.php';
        break;
    case 'grp':
    case 'groups':
    case 'staff':
        ...
        $nav->addSubMenu(array('desc' => 'Executive Members', 'href' => 'admin.php?t=staff', 'iconclass' => 'users'));
        $nav->addSubMenu(array('desc' => 'Add New Executive', 'href' => 'admin.php?t=staff&amp;a=new', 'iconclass' => 'newuser'));
        /*
    ...
    case 'client':
        $group = null;
        //Tab and Nav options.
        if ($_REQUEST['at'] == 'services') { //if request for other tab
            $target_tab = 'services';
            $nav->setTabActive('services');
        } else {
            $nav->setTabActive('client');
            $nav->addSubMenu(array('desc' => 'Clients', 'href' => 'admin.php?t=client', 'iconclass' => 'users'));
            $nav->addSubMenu(array('desc' => 'Add New Client', 'href' => 'admin.php?t=client&amp;a=new', 'iconclass' => 'newuser'));
        ...
        switch ($thistab) {
            ...
            case 'groups':
                if (($id = $_REQUEST['id'] ? $_REQUEST['id'] : $_POST['group_id']) && is_numeric($id)) {
                    $res = db_query('SELECT * FROM ' . CLIENT_GROUP_TABLE . ' WHERE group_id=' . db_input($id));
                    if (!$res or !db_num_rows($res) or !($group = db_fetch_array($res)))
                        $errors['err'] = 'Unable to fetch info on group ID#' . $id;
                }
                $page = ($group or ($_REQUEST['a'] == 'new' && !$gID)) ? 'group.inc.php' : 'groups.inc.php';
            ...
            case 'client':
                ...
                if ( ($id = $_REQUEST['id'] ? $_REQUEST['id'] : $_POST['client_id']) ) {
                    //  echo   'im here';
                    $client = new Client($id);
                    if (!$client || !is_object($client) || $client->getId() != $id) {
                        $client = null;
                        $errors['err'] = 'invalid id: ' . $id;
                    }
                }
                if ($client or (($_REQUEST['a'] == 'new') || ($_REQUEST['a'] == 'new_staff'))) {
                    $page = 'client.inc.php';
                } else {
                    $page = 'clientmembers.inc.php';
                }
                ...
                if ( $criteria['what'] && $criteria['type'] ) {
                    $page = 'clientmembers.inc.php';
                }
                break;
            default:
