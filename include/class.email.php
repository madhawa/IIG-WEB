...
class Email {
	...
	/*
	function getNOCmail() {
	    $sql = 'select email FROM ' . EMAIL_TABLE . ' WHERE email';
	} */
    ...
    function send($to,$subject,$message,$attachment=null, $cc = false, $custom_from=false, $custom_to=false) {
        global $cfg;
        ...
        //adding cc header, this will not change email recipients list, just edits the header
        if ( is_array($cc) ) {
            $cc = implode(',', $cc);
            $mime->addCc($cc);
        }
        ...
        //setting custom from header
        if ( $custom_from ) {
            if ( is_array($custom_from) ) {
                $custom_from = implode(',', $custom_from);
            }
            $mime->setFrom($custom_from);
        }
        
        //setting custom to header
        if ( $custom_to ) {
            if ( is_array($custom_to) ) {
                $custom_to = implode(',', $custom_to);
            }
            $mime->setTo($custom_to);
        }
