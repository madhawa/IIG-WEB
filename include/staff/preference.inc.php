...
$templates=db_query('SELECT tpl_id,name FROM '.EMAIL_TEMPLATE_TABLE.' WHERE cfg_id='.db_input($cfg->getId()));
 ?>
<div class="msg">System Preferences and Settings&nbsp;&nbsp;</div>
<table width="100%" border="0" cellspacing=0 cellpadding=0>
 ...
 <tr><td>
    ...
    <table width="100%" border="0" cellspacing=0 cellpadding=2 class="tform">
        <tr class="header"><td colspan=2>Autoresponders &nbsp;(Global Setting)(notification to client and noc )</td></tr>
        ...
        <tr><th valign="top">New Ticket:</th>
            <td><i>Autoresponse includes the ticket ID required to check status of the ticket</i><br>
                ...
                <br>
                a copy will also be delivered to noc department
            </td>
        </tr>
    ...
    <table width="100%" border="0" cellspacing=0 cellpadding=2 class="tform">
        ...
        <tr><th valign="top">New Ticket Alert:</th>
            <td>
                ...
                <?php  require_once INCLUDE_DIR.'class.email.php';  ?>
                <?php  if ( Email::getNOCmail() ) {  ?>
                    <input type="checkbox" name="ticket_alert_noc_dept" <?php    echo  $config['ticket_alert_noc_dept']?'checked':'' ?>> NOC department (this also determines new message alert)
                <?php  } else {  ?>
                    NOC email doesnot exist(email containing 'noc' string)
                <?php  }  ?>
                <!--
                <br>
                <br>
                ...
                <br>
                <?php 
                    //fetching more email lists for ticket alert
                    //getting only the first field from the custom settings database, no more rows required
                    /*
                    ...
                    * */
                 ?>
                <textarea name="more_ticket_alert_email"><?php    echo   $emails['more_ticket_alert_email'];  ?></textarea>
				-->
            </td>
        </tr>
 ...
 <tr>
    <td style="padding:10px 0 10px 240px;">
        <input class="button" name="save_settings" type="submit" name="submit" value="Save Changes">
        <input class="button" type="reset" name="reset" value="Reset Changes">
        <script type="text/javascript">
            $('input[name="save_settings"]').click(function(event) {
                var more_ticket_alert_mail = $('[name="more_ticket_alert_email"]').val();
                if ( more_ticket_alert_mail ) {
                    var mails = more_ticket_alert_mail.split(','); // array of cc emails
                    var invalid_mail = 0;
                    $.each(mails, function(index, value) { //looping
