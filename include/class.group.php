...
class Group {
    ...
    function save($id,$vars,&$errors) {
        ...
        if(!$errors){
            ...
            $sql=' SET updated=NOW(), group_name='.db_input(Format::striptags($vars['group_name'])).
                 ...
                 ', can_create_orders='.db_input($vars['can_create_orders']).
                 ', can_edit_orders='.db_input($vars['can_edit_orders']).
                 ', can_delete_orders='.db_input($vars['can_delete_orders']).
                 ', can_accept_reject_cancel_orders='.db_input($vars['can_accept_reject_cancel_orders']);
            //  echo   $sql;
            if($id) {
    ...
    function get_all_group_names() {
        ...
        if ( ($res = db_query($sql)) && db_num_rows($res) ) {
            $groups = db_assoc_array($res, true);
            foreach( $groups as $g ) {
                $grs[] = $g['group_name'];
            }
            return $grs;
