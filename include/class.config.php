...
class Config {
    ...
    function alertToNOC() {
        return $this->config['ticket_alert_noc_dept']?true:false;
    }
    ...
    function updatePref($var,&$errors) {
        ...
        $f['default_dept_id']=array('type'=>'int',   'required'=>0, 'error'=>'Default Dept. required');
        $f['default_email_id']=array('type'=>'int',   'required'=>0, 'error'=>'Default email required');
        $f['default_template_id']=array('type'=>'int',   'required'=>0, 'error'=>'You must select template.');
        $f['staff_session_timeout']=array('type'=>'int',   'required'=>0, 'error'=>'Enter idle time in minutes');
        $f['client_session_timeout']=array('type'=>'int',   'required'=>0, 'error'=>'Enter idle time in minutes');
        $f['time_format']=array('type'=>'string',   'required'=>0, 'error'=>'Time format required'); //TODO: Add date format validation  
        $f['date_format']=array('type'=>'string',   'required'=>0, 'error'=>'Date format required');
        $f['datetime_format']=array('type'=>'string',   'required'=>0, 'error'=>'Datetime format required');
        $f['daydatetime_format']=array('type'=>'string',   'required'=>0, 'error'=>'Day, Datetime format required');
        $f['admin_email']=array('type'=>'email',   'required'=>1, 'error'=>'Valid email required');
        $f['autolock_minutes']=array('type'=>'int',   'required'=>0, 'error'=>'Enter lock time in minutes');
        ...
        */
        ...
        $sql= 'UPDATE '.CONFIG_TABLE.' SET isonline='.db_input($var['isonline']).
            ...
            ',ticket_alert_noc_dept='.db_input(isset($var['ticket_alert_noc_dept'])?1:0).
