...
class ServiceOrder {
     ...
     * @return <bool>
     */
     //$id can be either order id or client name
    function load($id, $static_call = 0, $field_name = 'order_id') {
        switch($field_name) {
            case 'order_id':
            case 'id':
        ...
        $sql = sprintf('SELECT * FROM ' . ORDER_TABLE .
                ' WHERE %s=%s',$search_field, db_input($id));
        ...
        if (($res = db_query($sql)) && db_num_rows($res)) {
            ...
            if ( $search_field == 'customer_name' ) { //all orders for a customer
                $data = array();
                while ( $db_array = db_fetch_array($res) ) {
                    $data[] = $db_array;
                }
                return $data;
            } elseif ( $search_field == 'order_id' ) { //a single order by id
    ...
    */
    ...
    function create($vars, &$errors, $autorespond = true) {
        ...
        /*
        $fields = array();
        $fields['customer_rel_no'] = array('type' => 'string', 'required' => 1, 'error' => 'customer relationship no empty');
        $fields['client_id'] = array('type' => 'string', 'required' => 1, 'error' => 'client id empty');
        $fields['customer_name'] = array('type' => 'string', 'required' => 1, 'error' => 'customer name empty');
        $fields['customer_email'] = array('type' => 'email', 'required' => 1, 'error' => 'customer email empty');
        $fields['customer_type'] = array('type' => 'string', 'required' => 1, 'error' => 'customer type empty');
        $fields['service_type'] = array('type' => 'string', 'required' => 1, 'error' => 'service type empty');
        $fields['circuit_type'] = array('type' => 'string', 'required' => 1, 'error' => 'circuit type empty');

        $fields['order_creator_name'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person name empty');
        $fields['order_creator_designation'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person designation empty');
        $fields['order_creator_dept_name'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person department name empty');
        $fields['order_creator_address'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person address empty');
        $fields['order_creator_city'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person city name empty');
        $fields['order_creator_zip_or_po'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person zip code empty');
        $fields['order_creator_country'] = array('type' => 'string', 'required' => 1, 'error' => 'contact persona country name empty');
        $fields['order_creator_office_phone'] = array('type' => 'string', 'required' => 0, 'error' => 'contact person office phone number empty');
        $fields['order_creator_fax'] = array('type' => 'string', 'required' => 0, 'error' => 'contact person fax number empty');
        $fields['order_creator_mobile'] = array('type' => 'string', 'required' => 1, 'error' => 'contact person mobile number empty');
        $fields['order_creator_service_ready_date'] = array('type' => 'date', 'required' => 1, 'error' => 'ready for service date empty');
        //Customer End
        $fields['order_customer_name'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end contact person name empty');
        $fields['order_customer_designation'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end details contact person designation empty');
        $fields['order_customer_dept_name'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end details');
        $fields['order_customer_address'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end address empty');
        $fields['order_customer_city'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end city name empty');
        $fields['order_customer_zip_or_po'] = array('type' => 'int', 'required' => 1, 'error' => 'customer end zip code empty');
        $fields['order_customer_country'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end country name empty');
        $fields['order_customer_phone_office'] = array('type' => 'string', 'required' => 0, 'error' => 'customer end office phone number empty');
        $fields['order_customer_fax'] = array('type' => 'string', 'required' => 0, 'error' => 'customer end fax number empty');
        $fields['order_customer_mobile'] = array('type' => 'string', 'required' => 1, 'error' => 'customer end mobile number empty');
        $fields['order_customer_backhaul_provider'] = array('type' => 'string', 'required' => 1, 'error' => 'backhaul provider name empty');
        $fields['order_customer_backhaul_responsibility'] = array('type' => 'string', 'required' => 1, 'error' => 'backhaul responsibility empty');
        $fields['order_customer_equipment_to_be_used'] = array('type' => 'string', 'required' => 1, 'error' => 'select equipment to be used');
        $fields['order_customer_equipment_others'] = array('type' => 'string', 'required' => 0, 'error' => '*');
        $fields['order_customer_equipment_name'] = array('type' => 'string', 'required' => 1, 'error' => 'insert equipment name');
        $fields['order_customer_equipment_model'] = array('type' => 'string', 'required' => 1, 'error' => 'insert equipment model');
        $fields['order_customer_equipment_vendor'] = array('type' => 'string', 'required' => 1, 'error' => 'insert equipment vendor');
        $fields['order_customer_connectivity_interface'] = array('type' => 'string', 'required' => 1, 'error' => 'select connectivity interface');
        $fields['order_customer_connectivity_interface_others'] = array('type' => 'string', 'required' => 0, 'error' => '*');
        $fields['order_customer_protocol_to_be_used'] = array('type' => 'string', 'required' => 1, 'error' => 'select protocols to be used');
        $fields['order_customer_protocol_others'] = array('type' => 'string', 'required' => 0, 'error' => '');
        $fields['order_customer_connectivity_capacity'] = array('type' => 'string', 'required' => 1, 'error' => 'select connectivity capacity');
        ...
        $fields['order_technical_contact_name'] = array('type' => 'string', 'required' => 1, 'error' => 'technical contact name empty');
        $fields['order_technical_contact_mobile'] = array('type' => 'string', 'required' => 1, 'error' => 'technuical contact mobile number empty');
        $fields['order_technical_contact_phone'] = array('type' => 'string', 'required' => 0, 'error' => 'technical contact mobile phone number empty');
        $fields['order_technical_contact_email'] = array('type' => 'email', 'required' => 1, 'error' => 'technical contact emial id empty');
        $fields['order_technical_contact_messengers'] = array('type' => 'string', 'required' => 0, 'error' => 'technical contact messenger id empty');


        $fields['order_routing_type'] = array('type' => 'string', 'required' => 1, 'error' => 'select routing type');
        $fields['order_customer_as_sys_name'] = array('type' => 'string', 'required' => 1, 'error' => 'insert AS system name');
        $fields['order_customer_as_sys_num'] = array('type' => 'int', 'required' => 1, 'error' => 'insert AS number');
        $fields['order_customer_as_set_num'] = array('type' => 'int', 'required' => 1, 'error' => 'insert AS SET number');



        $fields['order_bgp_routing'] = array('type' => 'string', 'required' => 1, 'error' => 'seelct bgp routing');
        $fields['order_router_name'] = array('type' => 'string', 'required' => 1, 'error' => 'insert router name');
        $fields['order_bw_speed_cir'] = array('type' => 'int', 'required' => 1, 'error' => 'insert CIR BW speed');
        $fields['order_max_burstable_limit'] = array('type' => 'int', 'required' => 1, 'error' => 'insert max burstable limit');
        $fields['connectivity_interface'] = array('type' => 'string', 'required' => 1, 'error' => 'select connectivity interface');
        $fields['order_fiber_type'] = array('type' => 'string', 'required' => 1, 'error' => 'insert fiber type');
        $fields['order_ip_details_for_global'] = array('type' => 'string', 'required' => 0, 'error' => '*');
        $fields['order_special_routing_comments'] = array('type' => 'string', 'required' => 0, 'error' => '');


        $fields['order_billing_total_non_recurring_charges'] = array('type' => 'int', 'required' => 0, 'error' => '*');
        $fields['order_billing_total_monthly_recurring_charges'] = array('type' => 'int', 'required' => 0, 'error' => '*');
        $fields['order_billing_hw_charges'] = array('type' => 'int', 'required' => 0, 'error' => '*');
        ...
        $fields['order_billing_vat_or_tax'] = array('type' => 'int', 'required' => 0, 'error' => '*');
        $fields['order_billing_deposit'] = array('type' => 'int', 'required' => 0, 'error' => '*');
        $fields['order_billing_total_payable_with_sof'] = array('type' => 'int', 'required' => 0, 'error' => '* empty');


        $fields['order_special_requests_if_any'] = array('type' => 'text', 'required' => 0, 'error' => '');

        $fields['applicants_name'] = array('type' => 'text', 'required' => 1, 'error' => 'applicants name empty');
        $fields['applicants_designation'] = array('type' => 'text', 'required' => 1, 'error' => 'applicants designation empty');
        $fields['application_date'] = array('type' => 'text', 'required' => 1, 'error' => 'application date empty');
        ...
        if (!$validate->validate($vars)) {
            ...
            $errors['err'] = ' input fields problem ';
        }
        */
        ...
        $sql = ',customer_rel_no=' . db_input(Format::striptags($vars['customer_rel_no'])) .
        ...
        if ( !$vars['order_id'] ) {
            ...
            $sql = 'INSERT INTO ' . ORDER_TABLE .
                ...
                ',created_by=' . db_input($thisuser->getId()) .
                ',order_id=' . db_input($order_id) .
        ...
        if ( db_query($sql) && ( db_affected_rows() != -1 ) ) {
            ...
            if ( !$vars['order_id'] ) {
                ...
                Sys::log(LOG_INFO, 'new order created', $body);
                $order->responseEmail($subj, $body);
            } else {
                $subj = 'order updated';
                $body = 'order id:' . $order->getId() . ' updated, time ' . date('l jS \of F Y h:i:s A') . '(server time) from ip: ' . $ipaddress;
                Sys::log(LOG_INFO, 'order updated', $body);
                $order->responseEmail($subj, $body);
            }
        } else {
            //  echo   $sql.'<br>';
            //mysql_query($sql) or die(mysql_error());
            $errors['err'] .= ' failed to commit database query ';
        }
        return $order;
    }
